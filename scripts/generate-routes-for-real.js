...
function findMatchingRoutes (endpoint) {
  ...
  restEndpoints.forEach((route, i) => {
    ...
    if (route.method === endpoint.method && route.url === endpoint.path) {
      // matches.push(`${route.scope}.${normalize(route.idName)}`)
...
function normalize (methodName) {
  ...
  // }
  //
  // if (/^list[A-Z]/.test(methodName)) {
  //   return methodName.replace(/^list/, 'get')
  // }

  methodName = methodName.replace(/^edit/, 'update')
...
endpoints.forEach(endpoint => {
  ...
  const idName = normalize(_.camelCase(endpoint.idName))
